#CADENAS_CARACTERES_STRINGR
#Crea un vector de cadenas de caracteres con tu nombre y apellidos (por ejemplo, ["Rocío", "Romero", Zaliz"]). A partir de el crea una nueva cadena de caracteres con la inicial de tu nombre, un punto y tus apellidos (por ejemplo, "R. Romero Zaliz").
library(tidyverse)
nombre <-  c("Alejandro","Pérez", "Argüello")
nombre2 <- str_c(str_sub(nombre[1], 1, 1), ". ", nombre[2], " ", nombre[3])
nombre2
...
str_subset(fechas, "\\d+\\-[01][13579]\\-\\d+")
...
str_subset(frases2, "\\b[aeiouAEIOU]+\\b|\\b[bcdfghjklmnpqrstvwxyzBCDFGHJKLMNPQRSTVWXYZ]+\\b")
...
fechas <- as.Date(str_c(años, meses, dias, sep= "-"))
fechas[!is.na(fechas)]
