...
labs(title="Picos nevados", subtitle="Y no tan nevados", x="Año", y="cubierta de nieve de octubre para Eurasia (mill. km2)", caption="Fig. 1")
...
m <- matrix(c(2014,4,58,44,18,21.8,35.2,28.5,2.34,19.2,0.5,11,
...
1983,12,58,43,21,29.4,39.6,34.5,NA,1.9,NA,1,
...
1900,10,57,46,20,25.5,38.8,32.4,4.22,1,1.46,0.5), ncol=12, byrow=TRUE)
df <- data.frame(m)
colnames(df) <- c("Year","Lowest Temperature (F)","Highest Temperature (F)",
               "Warmest Minimum Temperature (F)","Coldest Minimum Temperature (F)",
               "Average Minimum Temperature (F)","Average Maximum Temperature (F)",
               "Mean Temperature (F)","Total Precipitation (in)",
               "Total Snowfall (in)","Max 24hr Precipitation (in)",
               "Max 24hr Snowfall (in)")

library(dplyr)

df2 <- df %>%
  mutate(
    Lowest_Temperature_C = (`Lowest Temperature (F)` - 32) * 5/9,
    Highest_Temperature_C = (`Highest Temperature (F)` - 32) * 5/9,
    Warmest_Minimum_Temperature_C = (`Warmest Minimum Temperature (F)` - 32) * 5/9,
    Coldest_Minimum_Temperature_C = (`Coldest Minimum Temperature (F)` - 32) * 5/9,
    Average_Minimum_Temperature_C = (`Average Minimum Temperature (F)` - 32) * 5/9,
    Average_Maximum_Temperature_C = (`Average Maximum Temperature (F)` - 32) * 5/9,
    Mean_Temperature_C = (`Mean Temperature (F)` - 32) * 5/9,
    Total_Precipitation_mm = `Total Precipitation (in)` * 25.4,
    Total_Snowfall_mm = `Total Snowfall (in)` * 25.4,
    Max_24hr_Precipitation_mm = `Max 24hr Precipitation (in)` * 25.4,
    Max_24hr_Snowfall_mm = `Max 24hr Snowfall (in)` * 25.4
  ) %>%
  select(
    Year, Lowest_Temperature_C, Highest_Temperature_C,
    Warmest_Minimum_Temperature_C, Coldest_Minimum_Temperature_C,
    Average_Minimum_Temperature_C, Average_Maximum_Temperature_C,
    Mean_Temperature_C, Total_Precipitation_mm,
    Total_Snowfall_mm, Max_24hr_Precipitation_mm,
    Max_24hr_Snowfall_mm
  )
 
ggplot(df2, aes(x = Year, y = Warmest_Minimum_Temperature_C)) +
geom_line() +
labs(title="Lineal representation of a broken accordion", subtitle="Warmer than ever", x="Year", y="Warmest minimum tempertature in C", caption="Fig. 1")
 ggplot(df2, aes(x = Year)) +
geom_line(aes(y = Warmest_Minimum_Temperature_C), color="red") +
geom_line(aes(y = Coldest_Minimum_Temperature_C), color="blue") +
labs(title="Lineal representation of two broken accordions", subtitle="It's 2 warm!", x="Year", y="Warmest and coldest minimum tempertature in C", caption="Fig. 1")
#4
diversity <- starwars %>% filter(!is.na(homeworld)) %>% group_by(homeworld) %>% summarise(diversity=n_distinct(species,na.rm = TRUE)) %>% arrange(desc(diversity))

ggplot(diversity, aes(y=diversity, x=reorder(homeworld, diversity))) +
geom_col() +
coord_flip() +
labs(title="Diversity of species by homeworld", subtitle="The galaxy's fragile ecosystems balance on monospecies planets, very sad!", x="Homeworld", y="Number of different species", caption="Fig. 1")
stats <- starwars %>% filter(!is.na(sex), !is.na(height), !is.na(mass), sex == "male" | sex == "female") %>% group_by(sex) %>% select(height, mass)
ggplot(stats, aes(x = height, color = sex, y = mass)) +
geom_point() +
scale_color_manual(values = c("red", "blue")) +
labs(title="Males are denser", subtitle="Sample size: Not enough", x="Height in cm", y="Mass in kg", color="Sex")
#5
 
set <- mtcars %>% group_by(cyl) %>% select(mpg, hp, wt)
ggplot(set, aes(y=factor(cyl), x=hp, size=wt, color=mpg)) +
geom_point() +
scale_color_gradient(low = "red", high ="green") +
labs(title="America's big powerful car obsession", subtitle="may come with a gas price", y="number of cylinders", x="horsepower", size="weight in 1000s of pounds", color="miles per gallon")
